...
//				+req.task_id.value
    ...
    public static int TransferMsg(Req_t req, String ip_input) throws UnknownHostException, IOException
    {
        ...
        while(true)
        {
            if(MainActivity.node.broadcast_allowed == 1)
            {
                MainActivity.node.broadcast_allowed = 0;
                for(int loop = 0; loop < MainActivity.node.outgoing_list.ip_list.size(); loop ++ )
                {
                   if(ip_input == MainActivity.node.outgoing_list.ip_list.get(loop))
                    {
                        continue;
                    }
                    ...
                    {
                        Socket socket = new Socket(MainActivity.node.outgoing_list.ip_list.get(loop), SystemParam.GotGlobalPort());
                        if(socket != null)
                        {
                ...
                MainActivity.node.broadcast_allowed = 1;
                break;
            }
        }
        return res;
        ...
        while(true)
        {
            if(MainActivity.node.broadcast_allowed == 1)
            {
                ...
                for(int loop = 0; loop < MainActivity.node.outgoing_list.ip_list.size(); loop ++ )
                {
                    ...
                    socket.connect(new InetSocketAddress(MainActivity.node.outgoing_list.ip_list.get(loop), SystemParam.GotGlobalPort()), 3000);
...
/*						DataOutputStream out = new DataOutputStream(socket.getOutputStream());
    ...
    public static boolean isPrevProcessed(Req_t req) throws IOException, NoSuchAlgorithmException
    {
        ...
        for(int loop = 0; loop < MainActivity.node.request_list.size(); loop ++)
        {
            byte[] hash_value2 = BasicCrypto.MyHash(MainActivity.node.request_list.get(loop).serialize());
    ...
    public static boolean isInICList(String ip_addr)
    {
        boolean res = false;
        {
            for(int loop = 0; loop < MainActivity.node.incoming_list.ip_list.size(); loop ++)
            {
                if(ip_addr == MainActivity.node.incoming_list.ip_list.get(loop))
                {
                    res = true;
                    break;
                }
    ...
    public static boolean isInOGList(String ip_addr)
    {
        boolean res = false;
        {
            for(int loop = 0; loop < MainActivity.node.outgoing_list.ip_list.size(); loop ++)
            {
                if(ip_addr == MainActivity.node.outgoing_list.ip_list.get(loop))
                {
                    res = true;
                    break;
                }
