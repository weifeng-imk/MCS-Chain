...
class Node_t implements Runnable
{
    ...
    //Just for Testing
    public ArrayList<Integer> just_for_test;
    public ArrayList<Integer> just_for_test_error;
    //Just for Testing
    ...
    public Node_t() throws  IOException, NoSuchAlgorithmException
    {
        ...
        //Just for Testing
        this.just_for_test = new ArrayList<Integer>();
        this.just_for_test_error = new ArrayList<Integer>();
        //Just for Testing
        ...
        //4=============================================================================
        //this.ip = core_network_tools.getLocalIpAddress(MainActivity.class);
        ...
        Log.println(Log.ERROR, "Error", "The ip and port should be updated!!!!!!");
        ...
        //6=========================================================================
        //Log.println(Log.ERROR, "Error", "Error in 6, testing");
    ...
    public void process_case999(Req_t req) throws NoSuchAlgorithmException {
        Log.println(Log.ERROR, "Block111", String.valueOf(this.just_for_test.size()));

        if(BasicCrypto.SignatureVerify1(req.content, req.pk_sender, req.Sig_from))
        {
            Log.println(Log.ERROR, "Block222", String.valueOf(this.just_for_test.size()));

            int int_temp = Integer.valueOf(req.content);
            int int_temp1 = int_temp %1000;
            int int_temp2 = (int_temp / 1000) % 1000;

            if(int_temp1 == int_temp2) {
                this.just_for_test.add(int_temp);
                Log.println(Log.ERROR, "Block", String.valueOf(this.just_for_test.size()));
                if(this.just_for_test.size() == 10) {
                    this.blockgen();
                }
            }
            else {
                this.just_for_test_error.add(int_temp);
            }
    ...
    public void process_case2(Req_t req) throws NoSuchAlgorithmException,  IOException, ClassNotFoundException
    {
        ...
        if(BasicCrypto.SignatureVerify1(BasicTool.preSignBlock(req), req.pk_sender, req.Sig_from))
        {
            ...
            if(block.no != MainActivity.node.block_list.size())
            {
                return;
            }
                ...
                try {
                    ...
                    for(int loop = 0; loop < this.task_list_curr.size();loop ++)
                    {
                        if(this.task_list_curr.get(loop).equals(data_6.task_id))
                        {
                            while(true)
                            {
                                if(MainActivity.node.data_read_allowed == 1)
                                {
                                    MainActivity.node.data_read_allowed = 0;
...
//
//									req_cast.Sig_from = BasicCrypto.MySignature1(msg_cast, MainActivity.node.node_info.sk, MainActivity.node.node_info.pk);
//									BasicTool.BroadcastMsg(req_cast);
                                    ...
                                    MainActivity.node.data_read_allowed = 1;
                                    break;
                                }
                            }
                            break;
                            ...
                            if(req.From.value.equals(this.worker_selected.get(loop).get(loop2).value))
                            {
                                while(true)
                                {
                                    ...
                                    if(this.data_write_allowed == 1)
                                    {
                                        ...
                                        for(int loop3 = 0; loop3 < MainActivity.node.data_hash_list.size(); loop3 ++){
                                            if(task_list_curr.get(loop3).task_id.value.equals(data1.task_id.value)){
                                                MainActivity.node.data_hash_list.get(loop3).add(data1);
                                                MainActivity.node.data_hash_list_record.get(loop3).add(0);
                                            }
                                        }
...
//			e.printStackTrace();
    ...
    public void blockgen()
    {
        if(this.checkBlockValue() == true)
        {
            ...
            if(MainActivity.node.isAll != 3)
            {
                MainActivity.node.setAll(2);

                System.out.println(MainActivity.node.waid
                        + "\tTrying to deal with the new found block");
                MainActivity.node.waid ++;
                ...
                try {
                    if(MainActivity.node.block_list.size() > 0)
                    {
                        hash_temp = BasicCrypto.MyHash(MainActivity.node.current_block_list.get(MainActivity.node.current_block_list.size() - 1).serialize());
                    }
                    else
                    {
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				//				for(int loop = 0; loop < MainActivity.node.bid_list.size(); loop ++)
//				{
//					for(int loop2 = 0; loop2 < MainActivity.node.bid_list.get(loop).size(); loop2 ++)
//					{
//						if(MainActivity.node.bid_list_record.get(loop).get(loop2) == 0)
//						{
//							byte[] byte_temp = null;
//							try {
//								byte_temp = BasicCrypto.MyHash(MainActivity.node.bid_list.get(loop).get(loop2).serialize());
//							} catch (NoSuchAlgorithmException e) {
//								System.out.println("Some error happened when starting mining");
//								WarningWindow ww = new WarningWindow("Some error happened when starting mining");
//								e.printStackTrace();
...
//				}
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				//				str_list_temp = new ArrayList<String>();
//				for(int loop = 0; loop < MainActivity.node.data_hash_list.size(); loop ++)
//				{
//					for(int loop2 = 0; loop2 < MainActivity.node.data_hash_list.get(loop).size(); loop2 ++)
//					{
//						if(MainActivity.node.data_hash_list_record.get(loop).get(loop2)== 0)
//						{
//							try
//							{
//								str_list_temp.add(new String(BasicCrypto.MyHash(MainActivity.node.data_hash_list.get(loop).get(loop2).serialize())));
//							} catch (NoSuchAlgorithmException e) {
//								System.out.println("Error");
//								WarningWindow ww = new WarningWindow("Some error happened when starting mining");
//								e.printStackTrace();
...
//				for(int loop = 0; loop < MainActivity.node.feedback_list_collected.size(); loop ++)
//				{
//					for(int loop2 = 0; loop2 < MainActivity.node.feedback_list_collected.get(loop).size(); loop2 ++)
//					{
//						if(MainActivity.node.feedback_list_collected_record.get(loop).get(loop2)== 0)
//						{
//							try {
//								str_list_temp.add(new String(BasicCrypto.MyHash(MainActivity.node.feedback_list_collected.get(loop).get(loop2).serialize())));
//							} catch (NoSuchAlgorithmException e) {
//								System.out.println("Error");
//								WarningWindow ww = new WarningWindow("Some error happened when starting mining");
//								e.printStackTrace();
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//				for(int loop = 0; loop < MainActivity.node.state_task_list.size(); loop ++)
//				{
//					try {
//						str_list_temp.add(new String (BasicCrypto.MyHash(String.valueOf(MainActivity.node.state_task_list.get(loop)))));
//					} catch (NoSuchAlgorithmException e) {
//						System.out.println("Error");
//						WarningWindow ww = new WarningWindow("Some error happened when starting mining");
//						e.printStackTrace();
...
//							str_list_temp.add(this.tak
				...
				*///Back up
                //ArrayList<Integer> int_list_temp = new ArrayList<Integer>();
                for(int loop = 0; loop < 10; loop ++) {
                    int int_temp = this.just_for_test.get(0);
                    int int_temp2 = 0;
                    for(int loop1 = 1; loop1 < 10 - loop; loop1 ++) {
                        if(int_temp > this.just_for_test.get(loop1)) {
                            int_temp2 = loop1;
                            int_temp = this.just_for_test.get(loop1);
                        }
                    }
                    this.just_for_test.remove(int_temp2);
                    block.just_for_test.add(int_temp);
                    int_temp = 0;
                    int_temp2 = 0;
...
//				for(int loop = 0; loop < MainActivity.node.worker_selected.size(); loop ++)
//				{
//					for (int loop2 = 0; loop2 < MainActivity.node.worker_selected.get(loop).size(); loop2 ++)
//					{
//						if(MainActivity.node.worker_selected_record.get(loop).get(loop2) == 0)
//						{
//							try {
//								str_list_temp.add(new String(BasicCrypto.MyHash(MainActivity.node.worker_selected.get(loop).get(loop2).value)));
//							} catch (NoSuchAlgorithmException e) {
//								System.out.println("Error");
//								WarningWindow ww = new WarningWindow("Some error happened when starting mining");
//								e.printStackTrace();
...
//				}
                ...
                block.num_block_obtain = MainActivity.node.num_block_obtain;
                MainActivity.node.num_block_obtain ++;
                block.pk = MainActivity.node.node_info.pk;
                block.tv = MainActivity.node.node_info.tv;
                block.hash_prev = new String(hash_temp);
                block.no = MainActivity.node.block_list.size();
                block.id.value = new String(MainActivity.node.node_info.id.value);
                ...
                try {
                    block.sig = BasicCrypto.MySignature1(str_temp_all, MainActivity.node.node_info.sk, MainActivity.node.node_info.pk);
                } catch (NoSuchAlgorithmException e) {
                    System.out.println("Error");
                    //WarningWindow ww = new WarningWindow("Some error happened when starting mining");
                    e.printStackTrace();
                ...
                MainActivity.node.setAll(1);
                this.pay_decided_total.value = 0.0;
                this.just_for_test.clear();
                ...
                try {
                    req_block.Sig_from = BasicCrypto.MySignature1(msg, MainActivity.node.node_info.sk, MainActivity.node.node_info.pk);
                } catch (NoSuchAlgorithmException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                }
    ...
    public void block_select() throws ClassNotFoundException, IOException, NoSuchAlgorithmException
    {
        ...
        else
        {
            ...
            temp = temp - time0;
            if((temp > 6000)&&(MainActivity.node.undecided_block_list.size() != 0))
            {
                this.time0 = 0;
                Block_t block = new Block_t();
                if(MainActivity.node.undecided_block_list.size() == 1)
                {
                    block = MainActivity.node.undecided_block_list.get(0);
                }
                else if(MainActivity.node.undecided_block_list.size() == 2)
                {
